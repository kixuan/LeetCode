...
- **用途**：用于字符串比较、搜索特定字符或子串等基础操作。
- **技巧**：使用循环遍历字符串的每个字符。注意边界条件，如字符串的开始和结束。
...
- **用途**：快速查找、字符频率统计、模式匹配等。
- **技巧**：使用哈希表（如 `HashMap` 或 `HashSet`）存储字符及其频率或位置。

#### 4. 动态规划

- **用途**：解决如最长公共子序列、最长回文子串等更复杂的问题。
- **技巧**：构建一个表格或数组，逐步构建解决方案。

#### 5. 滑动窗口

- **用途**：解决如找出字符串中的所有字母异位词、最小覆盖子串等问题。
- **技巧**：维护一个滑动窗口，根据条件增加或减少窗口的大小。
...
- **用途**：转换数据类型，如字符串与数字之间的转换。
- **技巧**：使用 `Integer.parseInt()` 等方法进行基本类型转换。
...
首先先把句子分成单词split(" ")，然后就直接把单词倒着放进String就行

注意空格的处理，首尾的用trim()，中间的就直接判断（因为已经用strip分好类了）
